...
if (!function_exists('encodeHaxId')){
    function encodeHaxId($data)
    {
        $hashids = new Hashids("data", 20);
        $data = $hashids->encodeHex($data);
        return $data;
    }
}

if (!function_exists('decodeHaxId')){
    function decodeHaxId($data)
    {
        $hashids = new Hashids("data", 20);
        $data = $hashids->decodeHex($data);
        return $data;
    }
}
...
if (!function_exists('getTaskTypeName')){
    function getTaskTypeName($id)
    {
        return TaskType::where('id', $id)->pluck('name')->first();
    }
}
...
if (!function_exists('getTagId')){
    ...
    {
        return ((int) Tag::where('title', $data)->pluck('id')->first());
    }
}

if (!function_exists('changeDateToDay')){
    function changeDateToDay($date)
    {
        date_default_timezone_set("Asia/Kolkata");
        $to = Carbon::parse(strtotime($date));
        $from = Carbon::parse(strtotime(date('Y-m-d H:i:s')));
        $days = $to->diffInDays($from);
        if($days == 0)
        {
            $hour = $to->diffInHours($from);
            if($hour == 0){
                $day = $to->diffInMinutes($from)." Minutes";
            }else{
                $day = $hour." Hours";
            }
        }else{
...
function departmentname($id) {
  ...
  return  $department = Department::where('id',$id)->first();
	return $department;
}